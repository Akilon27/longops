...
The new version brings operations in other bases (and convertions) and the possibility to show digits being pulled-down in divisions (thanks to Y. D. X. in https://forum.typst.app/t/how-to-draw-a-vertical-line-across-grid-cells/6384/2). Two functions help-en and help-fr may be used to get help about a function or a parameter (ex: `#help-fr("division")` or `#help-en("detail(add-rounded)"))`. The manuals and the help functions are made with help from tidy.

[![MIT License](https://img.shields.io/badge/license-MIT-blue)](LICENSE.txt)
[![User Manual](https://img.shields.io/badge/manuel-fr.pdf-teal)][french manual]
...
```typ
#import "@preview/longops:0.2.0": addition
...
// list et solution:true
#addition(6834,5967,list:(2,9,11,16),solution:true)
```
...
```typ
#import "@preview/longops:0.2.0": soustraction
...
#soustraction(524, 187, list: (3, 6, 9), show-borrow:true,type-mask:"",)
#soustraction(524, 187, list: (3, 6, 9), solution: true,show-borrow:true,type-mask:"",solution-color:eastern)
```
...
```typ
#import "@preview/longops:0.2.0": multiplication
...
```typ
#import "@preview/longops:0.2.0": division

// Un chiffre en plus que dans a et avec des flèches pour montrer les chiffres qui descendent
#division(62821.64,11,extradigits:1,arrows: true,)
// Cycle entre parenthèses, mode anglais
#division(3.7,7,cycle:true,mode:"",fr:false)
// list de cases masquées
#division(432, 12, list: (0, 4))
// Afficher la solution pour les cases masquées ci-dessus
#division(432, 12, list: (0, 4), solution: true)
```
...
```typ
#import "@preview/longops:0.2.0": racine

#racine(24368,groups: true)
...
```typ
#import "@preview/longops:0.2.0": egalite-euclidienne, egalite-decimale, add-en-ligne, sous-en-ligne
...
```typ
#import "@preview/longops:0.2.0": *

// Addition en binaire
#addition(base: 2, "111,1", "110",show-base: true)
// Soustraction en hexadécimal (base 16) avec méthode française
#soustraction(base: 16, "1A3", "F4",show-base: true)
// Soustraction en base 16 avec méthode anglo-saxonne (emprunts)
#soustraction(base: 16, fr: false, canceled: true, "1A3", "F4")
// Multiplication en base 20 avec retenues
#multiplication(base: 20, carries: true, "2A", "3F")
// En octal (Base 8), présentation anglo-saxonne, avec cycle et flèches:
#division(base: 8, "754", "12", fr:false, arrows: true, cycle: true, show-base: true)
```

<div align="center">
  <img alt="exemples de calculs dans d'autres bases" src="Exemples/bases.svg" style="max-width: 100%; padding: 10px 10px; box-shadow: 1pt 1pt 10pt 0pt #AAAAAA; border-radius: 4pt; box-sizing: border-box; background: white">
</div>

```typ
#import "@preview/longops:0.2.0": etapes-calcul
...
```typ
#import "@preview/longops:0.2.0": detail

#detail("A=15/21=(5*r3)/(7*r3)=5/7", add-rounded: true, vertical: true, c: "circle")
#detail(
...
```typ
#import "@preview/longops:0.2.0": addition-durees
...
#test(
  ...
  addition-durees((1, 23, 59, 59), (0, 0, 0, 1), list:(1,9,),solution: true)
)
#test(
...
```typ
#import "@preview/longops:0.2.0": soustraction-durees
...
More on these functions in the [french manual](Exemples/manuel-longops.pdf) or the [english manual](Exemples/longops-manual.pdf).
...
## Changelog
*0.2.0* 
- the four basic operations now work in bases from 2 to 36
...
[french manual]: Exemples/manuel-longops.pdf
[english manual]: Exemples/longops-manual.pdf
